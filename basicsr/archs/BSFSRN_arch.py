...
class BSFSRN(nn.Module):
    def __init__(self):
        ...
        self.conv3 = BSRN(num_in_ch=3, num_feat=64, num_block=8, num_out_ch=3, upscale=2,
                 conv='BSConvU', upsampler='pixelshuffledirect', p=0.25)
        self.GELU = nn.GELU()
        self.pre_upsampler1 = Upsamplers.PixelShuffleDirect(scale=2, num_feat=64, num_out_ch=3)
        self.pre_upsampler2 = Upsamplers.PixelShuffleDirect(scale=2, num_feat=3, num_out_ch=3)
        self.pre_upsampler3 = Upsamplers.PixelShuffleDirect(scale=2, num_feat=3, num_out_ch=3)
    def forward(self, x):
        # print("x_shape:".ljust(15),x.shape)
        out2up_1 = self.GELU(self.conv_input(x))
        # print(out2up_1.shape)
        out_I1 = self.pre_upsampler1(out2up_1)
        # print("out_I1:".ljust(15),out_I1.shape)
        out_S1 = self.conv1(x)# todo:输出的直接是转换为3通道的，可以修改输出为64通道
        # print("out_S1:".ljust(15),out_S1.shape)
        # out_R1 = self.convt_R1(out_I1)
        # # print("out_R1:".ljust(15),out_R1.shape)
        SR_2x = out_S1+out_I1
        # print("SR_2x:".ljust(15),SR_2x.shape)

        out_I2 = self.pre_upsampler2(SR_2x)
        # print("out_I2:".ljust(15),out_I2.shape)
        out_S2 = self.conv2(SR_2x)
        # print("out_S2:".ljust(15),out_S2.shape)
        # out_R2 = self.convt_R2(out_I2)
        # print("out_R2:".ljust(15),out_R2.shape)
        SR_4x = out_I2+out_S2

        out_I3 = self.pre_upsampler3(SR_4x)
        out_S3 = self.conv3(SR_4x)
