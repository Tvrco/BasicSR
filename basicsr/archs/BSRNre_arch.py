...
class BSDB(nn.Module):
    def __init__(self, in_channels, out_channels, conv=nn.Conv2d, p=1):
        ...
        self.esa = ESA(in_channels, conv=BSConvU)
    ...
    def forward(self, input):
        ...
        out_fused = self.esa(out_fused)
        out_fused = self.cca(out_fused)
        return out_fused + input


# @ARCH_REGISTRY.register()
class BSRNre(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=64, num_block=8, num_out_ch=3, upscale=4,
                 conv='BSConvU', upsampler='pixelshuffledirect', p=1):
        super(BSRNre, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvU':
            self.conv = BSConvU
        elif conv == 'BSConvre':
        ...
        self.B1 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B2 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B3 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B4 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B5 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B6 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B7 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B8 = BSDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
    ...
    def forward(self, input):
        ...
        # print(out_fea.shape) #torch.Size([1, 64, 16, 16])
...
if __name__ == "__main__":
    model = BSRNre(upscale=4,conv='BSConvU')
